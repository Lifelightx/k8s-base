...
export default function LoginPage({ onSuccess, onSignup, onBack }) {
  ...
  return (
    <div className="auth-page min-h-screen w-full flex items-center justify-center bg-bg relative overflow-hidden">
      ...
      <div className="bg-surface/60 backdrop-blur-xl border border-border2 rounded-[28px] p-10 w-full max-w-md shadow-[0_24px_64px_rgba(0,0,0,0.4)] relative z-10">
        {/* Logo */}
        ...
        <p className="text-sm text-text2 mb-8">Sign in to access your notes</p>
        ...
        <form id="form-login" onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
          <div className="flex flex-col gap-1.5">
            <label htmlFor="login-email" className="text-xs font-semibold text-text2 uppercase tracking-wider">Email</label>
            <input id="login-email" type="email" className={inputCls} placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)} autoComplete="email" autoFocus />
          </div>
          <div className="flex flex-col gap-1.5">
            <label htmlFor="login-password" className="text-xs font-semibold text-text2 uppercase tracking-wider">Password</label>
            <input id="login-password" type="password" className={inputCls} placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} autoComplete="current-password" />
          </div>
          <button
            id="btn-login-submit"
            type="submit"
        ...
        <p className="mt-8 text-center text-sm text-text3">
          ...
          <button id="btn-goto-signup" onClick={onSignup} className="text-accent font-semibold hover:text-accent2 transition-colors duration-200 bg-transparent border-none cursor-pointer">
            Sign up free
          </button>
        </p>
      </div>
